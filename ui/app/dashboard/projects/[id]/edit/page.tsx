...
export default function ProjectEdit({
  ...
  useEffect(() => {
    ...
    fetch(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/projects/${projectId}/page_templates?` +
        new URLSearchParams({
          page: String(0),
          limit: String(100),
          sort_field: "name",
  ...
  async function handleExportProject() {
    const data = await fetch(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/projects/${projectId}/export`,
      {
        method: "post",
        headers: {
          Authorization: getToken(),
  ...
  return (
    <div>
      {project && (
        <div className="m-2 flex justify-between rounded-md border p-2">
