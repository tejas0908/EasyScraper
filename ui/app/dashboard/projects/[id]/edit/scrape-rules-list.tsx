...
export function ScrapeRulesList({
  pageTemplate,
}: {
  ...
  useEffect(() => {
    fetch(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/projects/${pageTemplate.project_id}/page_templates/${pageTemplate.id}/scrape_rules?` +
        new URLSearchParams({
      ...
      <div className="grid grid-cols-5 gap-4 p-4">
        {scrapeRules &&
          scrapeRules.map((scrapeRule) => (
            <div
              key={scrapeRule.id}
              className="flex flex-row items-center justify-between rounded-md border p-2 shadow transition duration-300 hover:bg-slate-100"
            >
              <div className="truncate">{scrapeRule.alias}</div>
              <EditScrapeRuleSheet
