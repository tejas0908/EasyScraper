...
export function EditPageTemplateSheet({
  pageTemplate,
  pageTemplates,
  project,
  parentForceUpdate,
  ...
  const [outputType, setOutputType] = useState(
    pageTemplate ? pageTemplate.output_type : "LEAF",
  );
  const [scraper, setScraper] = useState(
    pageTemplate ? pageTemplate.scraper : "XPATH_SELECTOR",
  );
  const [outputPageTemplateId, setOutputPageTemplateId] = useState(
    pageTemplate ? pageTemplate.output_page_template_id : "",
  );
  const [exampleUrl, setExampleUrl] = useState(
    pageTemplate ? pageTemplate.example_url : "",
  );
  const [aiPrompt, setAiPrompt] = useState(
    pageTemplate ? (pageTemplate.ai_prompt ? pageTemplate.ai_prompt : "") : "",
  );
  const [aiInput, setAiInput] = useState(
    pageTemplate ? pageTemplate.ai_input : "TEXT",
  );
  ...
  function handleOutputPageChange(e: string) {
    if (e != null) {
      ...
      registerError(
        "outputPageTemplate",
        "Output Page Template is mandatory if Output Type is Page Source",
      );
    }
  ...
  async function handleDeletePageTemplate() {
    ...
    const data = await fetch(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/projects/${project.id}/page_templates/${id}`,
      {
        method: "delete",
        headers: {
          "Content-Type": "application/json",
      ...
      <SheetContent className="overflow-y-auto">
        ...
        <div className="space-y-2 p-2">
          <div>
            ...
            ></Input>
            <div className="text-xs text-red-500">{error.name}</div>
          </div>
          <div>
            ...
            <div className="mt-2 flex items-center space-x-2 rounded-lg border bg-slate-100 p-2 dark:bg-slate-500">
              ...
              <div className="flex flex-col">
                <span className="text-xs text-slate-700 dark:text-white">
                  <b>Page Source</b> = scraping this page results in urls of
                  further pages to be scraped
                </span>
                <span className="text-xs text-slate-700 dark:text-white">
                  <b>Leaf</b> = scraping this page results in actual data
                </span>
              </div>
            </div>
          </div>
          <div>
            ...
            <div className="mt-2 flex items-center space-x-2 rounded-lg border bg-slate-100 p-2 dark:bg-slate-500">
              ...
              <div className="flex flex-col">
                <span className="text-xs text-slate-700 dark:text-white">
                  <b>Xpath Selector</b> = use xpaths to scrape
                </span>
                <span className="text-xs text-slate-700 dark:text-white">
                  <b>CSS Selector</b> = use css selectors to scrape
                </span>
                <span className="text-xs text-slate-700 dark:text-white">
                  <b>AI Scraper</b> = LLMs are used to scrape{" "}
                </span>
              </div>
            </div>
          </div>
          ...
          {scraper == "AI_SCRAPER" && (
            <div>
              ...
              <div className="mt-2 flex items-center space-x-2 rounded-lg border bg-slate-100 p-2 dark:bg-slate-500">
                ...
                <div className="flex flex-col">
                  <span className="text-xs text-slate-700 dark:text-white">
                    The system prompt which will be sent to the LLM while
                    scraping the page
                  </span>
                </div>
              </div>
          ...
          {scraper == "AI_SCRAPER" && (
            <div>
              ...
              <div className="mt-2 flex items-center space-x-2 rounded-lg border bg-slate-100 p-2 dark:bg-slate-500">
                ...
                <div className="flex flex-col">
                  <span className="text-xs text-slate-700 dark:text-white">
                    <b>Html</b> = The full html will be sent to the LLM (costly)
                  </span>
                  <span className="text-xs text-slate-700 dark:text-white">
                    <b>Text</b> = Only the text content of the page is sent to
                    the LLM (cheaper)
                  </span>
                </div>
              </div>
          ...
          {outputType == "PAGE_SOURCE" && (
            <div>
              <Label>Output Page Template</Label>
              <Select
                value={outputPageTemplateId}
                onValueChange={handleOutputPageChange}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Output Page Template" />
                </SelectTrigger>
                <SelectContent
                  className={error.outputPageTemplate ? "border-red-500" : ""}
                >
                  {pageTemplates.map((pt) => (
                    <SelectItem
                      ...
                      value={pt.id}
                      disabled={
                        pageTemplate != null && pt.id == pageTemplate.id
                      }
                    >
                      {pt.name}
                    </SelectItem>
              ...
              <div className="text-xs text-red-500">
                {error.outputPageTemplate}
              </div>
              <div className="mt-2 flex items-center space-x-2 rounded-lg border bg-slate-100 p-2 dark:bg-slate-500">
                <CircleHelp className="h-4" />
                <div className="flex flex-col">
                  <span className="text-xs text-slate-700 dark:text-white">
                    Which page template is produced by scraping this page?
                  </span>
                </div>
              </div>
            </div>
        ...
        <SheetFooter className="px-2 pt-4">
          <Button
            ...
            disabled={pendingSave || hasErrors() || name.length == 0}
          >
          ...
          <Button
            variant="destructive"
            onClick={handleDeletePageTemplate}
